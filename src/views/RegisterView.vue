<template>
  <div class="container">
    <form class="form" @submit.prevent="submit">
      <p class="title">Registro de nuevo usuario</p>
      <div class="form-camp">
        <label for="name">Nombre:</label>
        <input v-model="name" type="text" id="name" @input="validateName" />
        <p v-if="errors.some((error) => error.field === 'name')" class="error">
          {{ errors.find((error) => error.field === 'name')?.message }}
        </p>
      </div>
      <div class="form-camp">
        <label for="group">Grupo:</label>
        <input v-model="group" type="text" id="group" @input="validateGroup" />
        <p v-if="errors.some((error) => error.field === 'group')" class="error">
...
const validateName = () => {
  if (!name.value) {
    ...
    errors.value = errors.value.filter((error) => error.field !== 'name')
  }
}

const validateGroup = () => {
  if (!group.value) {
    errors.value.push({ field: 'group', message: 'El grupo es requerido' })
  } else {
    errors.value = errors.value.filter((error) => error.field !== 'group')
  }
}
